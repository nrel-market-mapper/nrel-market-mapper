...
RSpec.describe "GET /api/v1/summaries?state=CA" do
  it "returns a list of PV data for a specific state by year" do
    get "/api/v1/summaries?state=CA"

    json = JSON.parse(response.body)

    expected = {
      "years"    => ["2007", "2008", "2009", "2010", "2011", "2012", "2013", "2014", "2015", "2016"],
      "cost"     => [8.359, 8.559, 8.255, 7.385, 6.856, 5.782, 4.957, 4.724, 4.693, 4.495],
      "installs" => [17873, 21316, 30117, 39448, 46726, 40639, 65382, 25820, 7, 2],
      "capacity" => [234.4466, 290.6396, 383.2033, 578.6522, 598.1639, 705.1962, 737.2442, 316.2762, 0.2076, 0.1513]
    }
